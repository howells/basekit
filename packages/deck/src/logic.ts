...
const toUint32 = (value: number): number => ((value % UINT32_RANGE) + UINT32_RANGE) % UINT32_RANGE;

/** Folds the high half of a 32-bit value into the low half so every character reaches every bit. */
const fold = (value: number): number => toUint32(value + Math.floor(value / 65_536));

/**
 * Unsigned multiplicative hash over the id with a final mix. The earlier form
 * took `Math.abs` of a signed product, which capped the result at 2^31, so
 * every id landed in the lower half of the range and every card tilted the
 * same way; it also left ids that differ only in their last characters nearly
 * identical. Arithmetic only: the lint config forbids bitwise operators.
 */
const hashString = (value: string): number => {
  let hash = 2_166_136_261;

  for (let index = 0; index < value.length; index += 1) {
    hash = toUint32(Math.imul(fold(hash + (value.codePointAt(index) ?? 0)), 16_777_619));
  }

  hash = toUint32(Math.imul(fold(hash), 2_654_435_761));
  hash = toUint32(Math.imul(fold(hash), 2_246_822_507));
